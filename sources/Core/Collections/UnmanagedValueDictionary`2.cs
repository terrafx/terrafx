...
[DebuggerTypeProxy(typeof(UnmanagedValueDictionary<,>.DebugView))]
public partial struct UnmanagedValueDictionary<TKey, TValue>
    : IEnumerable<(TKey Key, TValue Value)>,
      IEquatable<UnmanagedValueDictionary<TKey, TValue>>
    ...
    where TValue : unmanaged
{
    ...
    private UnmanagedArray<int> _buckets;
    private UnmanagedArray<Entry> _entries;

    private int _count;
    private int _freeCount;
    private int _freeList;

    /// <summary>Initializes a new instance of the <see cref="UnmanagedValueDictionary{TKey, TValue}" /> struct.</summary>
    public UnmanagedValueDictionary()
    ...
    public UnmanagedValueDictionary(nuint capacity)
    ...
    public UnmanagedValueDictionary(IEqualityComparer<TKey>? comparer)
    ...
    /// <exception cref="ArgumentOutOfRangeException"><paramref name="capacity" /> is <c>negative</c>.</exception>
    /// <exception cref="OverflowException"><paramref name="capacity" /> is greater than <see cref="int.MaxValue" />.</exception>
    public UnmanagedValueDictionary(nuint capacity, IEqualityComparer<TKey>? comparer)
    {
        _buckets = [];
        _entries = [];
        ...
        if (capacity != 0)
        {
            Initialize(checked((int)capacity));
        }

        // If no comparer is provided, or if the default is provided, we'd prefer to use
    ...
    public readonly nuint Count => (uint)(_count - _freeCount);
    ...
    public readonly nuint Capacity
    {
        get
        {
            ...
            return !entries.IsNull ? entries.Length : 0;
        }
    }
    ...
    /// <summary>Compares two <see cref="UnmanagedValueDictionary{TKey, TValue}" /> instances to determine equality.</summary>
    ...
    public static bool operator ==(UnmanagedValueDictionary<TKey, TValue> left, UnmanagedValueDictionary<TKey, TValue> right)
    {
    ...
    public static bool operator !=(UnmanagedValueDictionary<TKey, TValue> left, UnmanagedValueDictionary<TKey, TValue> right) => !(left == right);
    ...
    public void Clear()
    {
        ...
        {
            _buckets.Clear();
            _entries.AsSpan(0, (uint)count).Clear();
    ...
    private readonly void CopyTo(UnmanagedSpan<(TKey Key, TValue Value)> destination)
    {
        var count = _count;
        ...
        for (var i = 0; i < count; i++)
        {
            ...
            if (entry.Next >= -1)
            {
                destination.GetReferenceUnsafe((uint)i) = (entry.Key, entry.Value);
            }
        }
    }
    ...
    public void EnsureCapacity(nuint capacity)
    {
        ...
        if (capacity > currentCapacity)
        {
            ...
            Resize(HashUtilities.GetPrime(newCapacity));
        }
    }

    /// <inheritdoc />
    public override readonly bool Equals([NotNullWhen(true)] object? obj) => (obj is UnmanagedValueDictionary<TKey, TValue> other) && Equals(other);

    /// <inheritdoc />
    public readonly bool Equals(UnmanagedValueDictionary<TKey, TValue> other) => this == other;
    ...
    public bool Remove(TKey key, [MaybeNullWhen(false)] out TValue value)
    {
        ...
        if (existing)
        {
            ...
            {
                ref var lastEntry = ref _entries.GetReferenceUnsafe((uint)lastIndex);
    ...
    private readonly ref int GetBucketReference(UnmanagedSpan<int> buckets, int hashCode)
    {
    ...
    private readonly ref Entry GetEntryReference(TKey key, out int lastIndex)
    {
        ...
        if (!buckets.IsNull)
        {
            ...
            if (typeof(TKey).IsValueType && (comparer is null))
            {
                ...
                for (var i = GetBucketReference(buckets, hashCode) - 1; (uint)i < entries.Length; i = entry.Next)
                {
    ...
    private ref Entry GetEntryReference(TKey key, out bool existing)
    {
        ...
        {
            index = _freeList;
            entry = ref entries.GetReferenceUnsafe((uint)_freeList);
    ...
    private void Initialize(int capacity)
    {
        ...
        var entries = new UnmanagedArray<Entry>((uint)size);
    ...
    private void Resize(int newCapacity)
    {
        ...
        var newEntries = new UnmanagedArray<Entry>((uint)newCapacity);
