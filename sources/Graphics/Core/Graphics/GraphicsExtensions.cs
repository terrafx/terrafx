...
{
    private static readonly uint[] s_sizeMap = new uint[] {
        0,  // Unknown
    ...
    public static uint GetSize(this GraphicsFormat format)
    {
        Assert(AssertionsEnabled && (s_sizeMap.Length == Enum.GetValues<GraphicsFormat>().Length));
        return s_sizeMap[(uint)format];
    }
}
