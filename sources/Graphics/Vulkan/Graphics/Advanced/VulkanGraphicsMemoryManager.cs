...
public sealed unsafe class VulkanGraphicsMemoryManager : GraphicsMemoryManager
{
    ...
    internal VulkanGraphicsMemoryManager(VulkanGraphicsDevice device, in VulkanGraphicsMemoryManagerCreateOptions createOptions) : base(device)
    {
        ...
        _vkMemoryTypeIndex = createOptions.VkMemoryTypeIndex;
    ...
    private nuint GetAdjustedMemoryAllocatorByteLength(nuint byteLength)
    {
        ...
        if (memoryAllocatorByteLength < MaximumSharedMemoryAllocatorByteLength)
        {
            ...
            if (MinimumMemoryAllocatorByteLength != MaximumSharedMemoryAllocatorByteLength)
            {
                ...
                for (var index = 0; index < 3; ++index)
                {
                    ...
                    {
                        memoryAllocatorByteLength = Math.Max(smallerMemoryAllocatorByteLength, MinimumMemoryAllocatorByteLength);
                    }
                    else
                    {
                        break;
        ...
        static nuint GetLargestSharedMemoryAllocatorByteLength(Span<GraphicsMemoryAllocator> memoryAllocators)
        {
            ...
            for (var index = memoryAllocators.Length; index >= 0; index--)
            {
                ...
                if (memoryAllocatorByteLength < MaximumSharedMemoryAllocatorByteLength)
                {
                    result = Math.Max(result, memoryAllocatorByteLength);
                }
        ...
        else
        {
            ...
            while (byteLength < minimumByteLength)
            {
                if (memoryAllocatorCount < MaximumMemoryAllocatorCount)
                {
                    ...
                    if (((byteLength + memoryAllocatorByteLength) > minimumByteLength) && (memoryAllocatorByteLength != MinimumMemoryAllocatorByteLength))
                    {
